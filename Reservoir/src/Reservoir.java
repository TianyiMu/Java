...
public class Reservoir 
{
	...
	public void addTemp(ArrayList<Double> temp)
	{
		this.temp=temp;
	}
	...
	public OLSMultipleLinearRegression linearReg()
	{
		OLSMultipleLinearRegression regression = new OLSMultipleLinearRegression();
		double[] y = new double[volume.size()];
		for(int i=0;i<volume.size();i++)
		{
			y[i]=volume.get(i);
		}
		double[][] x = new double[avgPrecip.size()][];
		for(int i=0;i<avgPrecip.size()&&i<temp.size();i++)
		{
			x[i] = new double[]{avgPrecip.get(i),temp.get(i)};
		}
